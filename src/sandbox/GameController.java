...
//        this.MoveComp(CellsColors[currentDif - 1][currentDif - 1]);
        recursMove(CellsColors[0][0], 1, 0, 0);
        recursMove(CellsColors[currentDif - 1][currentDif - 1], 2, currentDif - 1, currentDif - 1);
        fieldcleaner();
    }
...
//        System.out.println("returned int - "+index_val);
    ...
    public static void setCurrentDif(int currentDif) {
        ...
        if (currentDif == Settings.max_hard.get()) {
            cellssize = 10;
        }
        if (currentDif == Settings.med_hard.get()) {
            cellssize = 20;
        }
        if (currentDif == Settings.min_hard.get()) {
            cellssize = 40;
        }
    }
    ...
    public void fieldcleaner() {
        for (int i = 0; i < currentDif; i++) {
            for (int j = 0; j < currentDif; j++) {
                if ((CellsPlayer[i][j] == 3)) {
                    CellsPlayer[i][j] = CellsPlayer[i][j] - 2;
                }
                if ((CellsPlayer[i][j] == 4)) {
                    CellsPlayer[i][j] = CellsPlayer[i][j] - 2;
                }
            }
        }
    ...
    public void recursMove(int a, int b, int i, int j) {
        if (a == -1) {
            a = goodMove();
        }

        if (CellsPlayer[i][j] == b) {
            CellsPlayer[i][j] = b + 2;
            CellsColors[i][j] = a;
        }
        if ((j  < currentDif -1) && (CellsPlayer[i][j + 1] == 0) && (CellsColors[i][j + 1] == a)) {
            CellsPlayer[i][j + 1] = b;
            recursMove(a, b, i, j + 1);
        } else if ((j  < currentDif -1) && (CellsPlayer[i][j + 1] == b)) {
            recursMove(a, b, i, j + 1);
        }

        if ((i  < currentDif-1 ) && (CellsPlayer[i + 1][j] == 0) && (CellsColors[i + 1][j] == a)) {
            CellsPlayer[i + 1][j] = b;
            recursMove(a, b, i + 1, j);
        } else if ((i  < currentDif-1 ) && (CellsPlayer[i + 1][j] == b)) {
            recursMove(a, b, i + 1, j);
        }
        ...
        if ((j  >0) && (CellsPlayer[i][j - 1] == 0) && (CellsColors[i][j - 1] == a)) {
            CellsPlayer[i][j - 1] = b;
            recursMove(a, b, i, j - 1);
        } else if ((j  > 0) && (CellsPlayer[i][j - 1] == b)) {
            recursMove(a, b, i, j - 1);
        }
        ...
        if ((i  > 0) && (CellsPlayer[i - 1][j] == 0) && (CellsColors[i - 1][j] == a)) {
            CellsPlayer[i - 1][j] = b;
            recursMove(a, b, i - 1, j);
        } else if ((i >0) && (CellsPlayer[i - 1][j] == b)) {
            recursMove(a, b, i - 1, j);
        }
    }
